...
|------------|-----------------|---------|
| `https://www.mytriplea.co.za/` | `C:\WebApps\TripleA\` (site root) | Static gateway (`site-landing/`) — deployed by CI |
...
5. **HTTPS** certificates for `*.mytriplea.co.za` and `app.mytriple.co.za`
...
publish/iis/
  ...
  portal/   -> app.mytriple.co.za site root
  app/      -> owner.mytriplea.co.za site root
```

Copy each folder to `C:\WebApps\TripleA\` and bind to the matching IIS site. Optional: `-ApiBaseUrl https://www.mytriplea.co.za` (default).
...
Source for the gateway page: [`site-landing/`](../../site-landing/) (no build step). Gateway links point to `https://owner.mytriplea.co.za` and `https://app.mytriple.co.za`.
...
$env:VITE_API_BASE_URL="https://www.mytriplea.co.za"
npm run build -- --base /
```

Copy `dist\*` to `C:\WebApps\TripleA\portal`. IIS site binding: **app.mytriple.co.za**.

`public/web.config` is included in the build for SPA routing at site root.

### IIS: portal site

- Host: `app.mytriple.co.za`
...
- [ ] `https://www.mytriplea.co.za/` loads gateway with Owner App and Physio Portal buttons
- [ ] Gateway buttons navigate to `owner.mytriplea.co.za` and `app.mytriple.co.za`
...
- [ ] API auth: `POST https://www.mytriplea.co.za/api/auth/login` returns tokens
...
| 404 on portal routes | URL Rewrite installed; `web.config` in portal folder; site bound to `app.mytriple.co.za` |
| 404 on owner routes | Same for owner site at `owner.mytriplea.co.za` |
| CORS errors | `ASPNETCORE_ENVIRONMENT=Staging`; origins include `app.mytriple.co.za` and `owner.mytriplea.co.za` |
